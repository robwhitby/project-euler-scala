...
val names = io.Source.fromFile("names.txt").mkString.replace("\"", "").toUpperCase.split(",")

val total = names.sorted.zipWithIndex.map{ case(name, i) => name.map(_ - 64).sum * (i+1) }.sum
