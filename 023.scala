...
val abundants = (1 to max).filter(n => (1 to n/2).filter(n % _ == 0).sum > n)

val sums = abundants.view.flatMap { n => abundants.takeWhile(_ <= (28123 - n)).map(_ + n) } 

val sum = (1 to max diff sums).sum 
